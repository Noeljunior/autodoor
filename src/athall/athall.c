...
void ath_pin_setmode(pin * p, uint8_t mode) {
    ...
    if (mode & ATHP_ACTIVEHIGHT) {
        ath_pin_activehight(p);
    }
    /* set it as active low */
    if (mode & ATHP_ACTIVELOW) {
        ath_pin_activelow(p);
    }
    /* set it to high */
...
void ath_pin_activelow(pin * p) {
    if (p->mode == ATHP_INPUT) { /* enable pull-up registor */
        ath_pin_high(p);
    }
}

void ath_pin_activehight(pin * p) {
    if (p->mode == ATHP_INPUT) { /* disable pull-up registor */
        ath_pin_low(p);
    }
}

void ath_init_setmode(pin * p, volatile uint8_t *ddr, volatile uint8_t *port,
                        volatile uint8_t *pin, uint8_t bit, uint8_t mode) {
    /* init pin */
...
void ath_init_pwm(pin * p, ATHP_C tcounter, uint16_t top, uint16_t prescaler) {
    ...
    switch (tcounter) {
        ...
        /* 7-step prescaler Timer/Counter[01345] */
